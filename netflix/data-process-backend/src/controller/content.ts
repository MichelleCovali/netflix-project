...
export const postStartWatchMovie = async (req: Request & { user?: User }, res: Response): Promise<void> => {
    ...
    // TODO: Check if startTime is not longer than movie duration and  if the movieObject is finished, then start the movie from beginning  if its within 3 minutes of movie length then set the finished to true
    ...
    try {
        await db.tx(async t => {
            ...
            if (previousWatchHistoryObject === null || previousWatchHistoryObject.finished === true) {
                //If it doesn't exist then start the movie from beginning

                const watchHistoryObject = await t.one('INSERT INTO watch_history (profile_id, event_type, finished) VALUES ($<profileId>, $<eventType>, $<finished>) RETURNING watch_history_id', {
                    profileId: profileId,
                    eventType: 'Start',
                    finished: false
                })
...
export const postEndWatchMovie = async (req: Request & { user?: User }, res: Response): Promise<void> => {
    ...
    try {
        ...
        try {
            ...
            //Check if the movie watch history matches the movie so you won't be able to start a new movie and end a different movie
...
export const getWatchMovie = async (req: Request & { user?: User }, res: Response): Promise<void> => {
    ...
    //Make sure parameters are submitted
...
export const postStartWatchSeries = async (req: Request & { user?: User }, res: Response): Promise<void> => {
    ...
    try {
        ...
        //If there is a watch history entry then extract the information from the episode/season/series, make a table of them and save it into a variable that is outside the try catch block
    ...
    if the episode is finished then start the next episode,
    if the season is finished then start the next season, 
    if it's the last episode of the last season then start from the beginning
    */
    ...
    try {
        await db.tx(async t => {
            ...
            //Check if previous watch history does not exist so you can start the series from beginning
            if (previousWatchHistoryObject === null) {
                //If it doesn't exist then start the series from beginning
            ...
            // if previous watch history exists then first check if it's finished. If it's not then continue from the pause time
            ...
            // if previous watch history exists and its finished then check if it's the last episode of the season or if it's the last episode of the last season. If its not then start the next episode
...
export const postEndWatchSeries = async (req: Request & { user?: User }, res: Response): Promise<void> => {
    ...
    try {
        ...
        try {
            ...
            //Check if the series watch history matches the series so you will not be able to start a new series and end a different series
